...
# Whole path segments only: "/login", "/auth/", "/account/login?x" - not "/author/dan".
_LOGIN_RX = re.compile(r"/(?:%s)(?:/|$)" % "|".join(re.escape(h) for h in LOGIN_HINTS))
CONF_ORDER = {"High": 3, "Medium": 2, "Low": 1, "": 0}


def _as_text(v):
    """Engines hand back page bodies as str or bytes (Sherlock encodes r.text); always work on str."""
...
def _redirect_suspicious(req_url, final_url):
    ...
    try:
        ...
        return False
    ap = (a.path or "/").rstrip("/").lower() or "/"
    fp = (b.path or "/").rstrip("/").lower() or "/"
    if ap == fp:
        # Same page - only the scheme, www. or a trailing slash changed.
        return False
    if _LOGIN_RX.search(fp):
        return True
    if fp == "/" and ap != "/":
        return True
    return False
...
def own_check(name, info, username, timeout=15, proxy=None, ua=VERIFY_UA):
    ...
    # No redirect: report the profile URL, not the urlProbe address, so the
    # redirect heuristic never treats an API probe as "landed somewhere else".
    out["final_url"] = r.url if r.url != req_url else url
    text = r.text or ""
...
def score(row, info, flaky_set):
    ...
    if text:
        ...
        # Nothing to inspect (Maigret-only rows, or a Sherlock row before its page
        # text arrives): a bare status code is not enough for High.
...
def verify_row(row, info, timeout, proxy):
    ...
    if not info.get("url"):
        ...
        # (sites often lower-case the username in the URL, so match case-insensitively
        # and request the URL exactly as the engine reported it).
        m = re.search(re.escape(uname), row.get("url") or "", re.I)
        if not m:
            return "Unclear", {}
        uname = m.group(0)
        info = {"url": row["url"][:m.start()] + "{}" + row["url"][m.end():], "errorType": "status_code"}
    res = own_check(row["site"], info, uname, timeout=timeout, proxy=proxy)
    st = res["status"]
    if st == "WAF":
        return "Blocked", res
...
def variations(username, patterns=None, max_total=16):
    ...
    if lower != base:
        roots.append(lower)
    out, seen = [], set()
    # Pattern-major so every root (separator variants, lower-case form) gets the
    # plain "{u}" before the cap fills up with decorations of the first root.
    for p in pats:
        for r in roots:
            v = p.replace("{u}", r)
            k = v.lower()
        ...
        try:
            ...
            for site, r in (results or {}).items():
                ...
                try:
                    ...
                    hs = None
                enrich[site] = {"http_status": hs, "text": _as_text(r.get("response_text"))[:200000],
                                "url": r.get("url_user")}
            ...
            for site, r in (results or {}).items():
                ...
                for key in ("ids_data",):
                    ...
                    if isinstance(v, dict):
                        ids.update(v)
                enrich[site] = {
                    "http_status": hs, "text": _as_text(r.get("response_text") or r.get("html"))[:200000],
