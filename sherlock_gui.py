...
Image tab    : reverse image search links (Google Lens, Bing, Yandex, TinEye)
               and dHash matching against fetched profile pictures.
...
DEFAULTS = {
    ...
    "show_unknown": False, "show_desc": True, "category": "All", "dark": True,
...
def load_settings():
    ...
    try:
        with open(os.path.join(HERE, SETTINGS_FILE), "r", encoding="utf-8") as f:
            loaded = json.load(f)
        # Older settings files had "appearance": system/light/dark; only an explicit
        # "light" carries over - everything else gets the dark default.
        if "dark" not in loaded and "appearance" in loaded:
            loaded["dark"] = loaded.get("appearance") != "light"
        loaded.pop("appearance", None)
        s.update(loaded)
    except Exception:
        pass
    return s
...
def main():
    ...
    class App:
        def __init__(self, root):
            ...
            self.font_family = theme.ui_family(root)
            self.mono_family = theme.mono_family(root)
            self.font_size = 13 if sys.platform == "darwin" else 10

            self.dark = bool(self.S.get("dark", True))
            self.dark_var = tk.BooleanVar(value=self.dark)
            self.style = ttk.Style(root)
            self.P = theme.apply(root, self.dark)
            self.text_widgets = []
            self.edit_widgets = []      # editable tk.Text widgets (field colours)
            self.toplevels = []         # open dialogs, recoloured on toggle
        ...
        def toggle_dark(self, *_):
            self.dark_var.set(not self.dark)
            self.set_dark()

        def set_dark(self):
            self.dark = bool(self.dark_var.get())
            self.S["dark"] = self.dark
            save_settings(self.S)
            self._apply_theme()

        def _apply_theme(self):
            # House palette + ttk styles from theme.py, then the styles and classic
            # tk widgets this app has on top of it.
            P = self.P = theme.apply(self.root, self.dark)
            s = self.style
            base_font = (self.font_family, self.font_size)
            s.configure(".", font=base_font)
            s.configure("Treeview", rowheight=24, font=base_font)
            s.configure("Muted.TLabel", background=P["bg"], foreground=P["dim"])
            ...
            s.configure("TSpinbox", fieldbackground=P["field"], foreground=P["field_text"], arrowcolor=P["accent"],
                        insertcolor=P["accent"], bordercolor=P["field_border"], lightcolor=P["field_border"],
                        darkcolor=P["field_border"], background=P["panel"], relief="solid")
            s.map("TSpinbox", bordercolor=[("focus", P["accent"])], lightcolor=[("focus", P["accent"])],
                  darkcolor=[("focus", P["accent"])], fieldbackground=[("readonly", P["field"])],
                  foreground=[("readonly", P["field_text"])])
            s.configure("TMenubutton", background=P["panel"], foreground=P["text"], arrowcolor=P["accent"],
                        bordercolor=P["field_border"], lightcolor=P["field_border"], darkcolor=P["field_border"],
                        padding=(11, 6), relief="solid")
            s.map("TMenubutton", background=[("active", P["sel"]), ("disabled", P["bg"])],
                  foreground=[("disabled", P["dim"])], bordercolor=[("active", P["accent"]), ("focus", P["accent"])])
            ...
            s.configure("TLabelframe", background=P["bg"], bordercolor=P["border"], lightcolor=P["bg"],
                        darkcolor=P["bg"])
            s.configure("TLabelframe.Label", background=P["bg"], foreground=P["dim"])
            s.configure("TNotebook", tabmargins=(4, 4, 4, 0))
            s.configure("TNotebook.Tab", padding=(14, 6))
            self.text_widgets = [w for w in self.text_widgets if w.winfo_exists()]
            self.edit_widgets = [w for w in self.edit_widgets if w.winfo_exists()]
            for w in self.text_widgets:
                self._style_text(w, editable=False)
            for w in self.edit_widgets:
                self._style_text(w, editable=True)
            self.toplevels = [t for t in self.toplevels if t.winfo_exists()]
            for t in self.toplevels:
                t.configure(bg=P["bg"])
            # Sherlock-specific colours: confidence and result states from good/warn/bad/dim.
            for tree in (self.tree, self.etree):
                tree.tag_configure("odd", background=P["code"])
                ...
                tree.tag_configure("high", foreground=P["good"])
                tree.tag_configure("avail", foreground=P["dim"])
            try:
                for m in [self.menubar] + self.menus:
                    m.configure(bg=P["panel"], fg=P["text"], activebackground=P["sel"], activeforeground=P["text"],
                                disabledforeground=P["dim"])
            except Exception:
                pass
            self._render()

        def _style_text(self, w, editable):
            """Recolour a classic tk.Text from the palette (editable: field colours; read-only: panel)."""
            P = self.P
            w.configure(bg=P["field"] if editable else P["panel"], fg=P["field_text"] if editable else P["text"],
                        insertbackground=P["accent"], highlightbackground=P["field_border" if editable else "border"],
                        highlightcolor=P["accent"], highlightthickness=1, selectbackground=P["sel"],
                        selectforeground=P["text"])
            w.tag_configure("h", foreground=P["accent"], font=(self.font_family, self.font_size, "bold"))
            w.tag_configure("k", foreground=P["dim"])
            w.tag_configure("ok", foreground=P["good"])
        ...
        def _build_menu(self):
            ...
            fm.add_separator()
            fm.add_command(label="Default save folder...", command=self.choose_save_dir)
            fm.add_command(label="Reset save folder to Downloads", command=self.reset_save_dir)
            fm.add_separator()
            ...
            vm.add_checkbutton(label="Dark mode", variable=self.dark_var, command=self.set_dark,
                               accelerator="Cmd+D" if sys.platform == "darwin" else "Ctrl+D")
            self.menubar.add_cascade(label="View", menu=vm)
            self.root.bind("<Control-d>", self.toggle_dark)
            self.root.bind("<Command-d>", self.toggle_dark)
        ...
        def help_tabs(self):
            ...
            t = tk.Text(win, wrap="word", relief="flat", font=(self.font_family, self.font_size), padx=10, pady=8)
            self._style_text(t, editable=False)
            self.text_widgets.append(t)
        ...
        def _build_ui(self):
            ...
            # The log exists before the tabs are built so tab builders can report
            # (e.g. Pillow missing on the Image tab); it is packed below the notebook.
        ...
        def image_browse(self):
            path = filedialog.askopenfilename(title="Choose an image", initialdir=self._save_dir(),
        ...
        def _save_dir(self):
            """Where exports go: Settings 'save_dir' if it exists, else the Downloads folder."""
            return tools.default_save_dir(self.S)

        def choose_save_dir(self):
            d = filedialog.askdirectory(title="Default save folder", initialdir=self._save_dir(), mustexist=True)
            if d:
                self.S["save_dir"] = d
                save_settings(self.S)
                self.logmsg("Default save folder: %s" % d)

        def reset_save_dir(self):
            self.S["save_dir"] = ""
            save_settings(self.S)
            self.logmsg("Default save folder reset to %s" % self._save_dir())
            ...
            try:
                for _ in range(600):
                    ...
                    try:
                        ...
                        # One bad message must not stop the pump (the search would look hung).
                        self.logmsg("Internal error handling %r: %r" % (msg[0] if msg else msg, e))
            except queue.Empty:
                pass
        ...
        def _list_editor(self, title, intro, items, on_save, defaults):
            ...
            txt = tk.Text(win, wrap="none", relief="flat", font=(self.font_family, self.font_size))
            self._style_text(txt, editable=True)
        ...
        def email_export(self):
            ...
            path = filedialog.asksaveasfilename(defaultextension=".csv",
                                                ...
                                                initialdir=self._save_dir(), filetypes=[("CSV", "*.csv")])
            if not path:
                return
            try:
                with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        def help_fp(self):
            messagebox.showinfo("False-positive filter",
                ...
                "Verify re-requests each claimed URL (hidden rows included) with a browser-like User-Agent, applies the "
