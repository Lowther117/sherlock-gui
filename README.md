...
- several usernames at once, proxy support, CSV / TXT export buttons (JSON
  via File > Export), dark mode by default with a light mode (View > Dark mode,
  Ctrl+D / Cmd+D). Every site is always searched - there is no NSFW toggle.
...
4. **File > Export** writes the *visible* rows as CSV, JSON or a plain list
   of URLs. Save dialogs open in your Downloads folder unless you pick another
   under **File > Default save folder...** (stored in `sherlock_gui.json`).
...
- **High**: everything else. A row Verify confirms goes to High; if it had
  Low warnings (or its site is on the flaky list) it only rises to Medium.
...
```
sherlock-gui/
  sherlock_gui.py            the app (Tkinter GUI, five tabs)
