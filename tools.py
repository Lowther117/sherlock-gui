...
# ----------------------------------------------------------------------------
# Where user output goes (exports etc.)
# ----------------------------------------------------------------------------


def downloads_dir():
    """The user's Downloads folder (Windows: the real known folder), else home."""
    home = os.path.expanduser("~")
    if sys.platform.startswith("win"):
        try:
            import ctypes
            import uuid
            # FOLDERID_Downloads {374DE290-123F-4565-9164-39C4925E467B} as a 16-byte GUID struct
            fid = (ctypes.c_ubyte * 16)(*uuid.UUID("374DE290-123F-4565-9164-39C4925E467B").bytes_le)
            path = ctypes.c_wchar_p()
            if ctypes.windll.shell32.SHGetKnownFolderPath(fid, 0, None, ctypes.byref(path)) == 0:
                p = path.value
...
def default_save_dir(settings=None):
    """settings['save_dir'] if it exists on disk, otherwise the Downloads folder."""
    d = (settings or {}).get("save_dir") or ""
...
def mx_check(email):
    ...
    res = dns_lookup(dom, ["MX"])
    recs = res.get("MX") or []
    if "MX" not in res:
        # stdlib fallback cannot ask for MX records at all
        row["state"] = "Unknown"
        row["other"] = "dnspython not installed - MX records cannot be looked up"
    elif recs and not str(recs[0]).startswith("error"):
                ...
                if reg.get("created"):
                    try:
                        age = (datetime.now(timezone.utc).replace(tzinfo=None)
                               - datetime.strptime(reg["created"], "%Y-%m-%d")).days
